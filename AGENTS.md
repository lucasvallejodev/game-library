...
12. **Gate protected routes with `app.requireAuth` on `onRequest`, never `preHandler`.** Fastify
    validates the body before `preHandler`, so an anonymous request with a bad payload would get
    a 422 disclosing the route's schema instead of a 401.
13. **Never run `git commit`, `git push`, or `git tag`.** At the end of each roadmap phase,
...
14. **Never hand-edit the Better Auth tables** (`user`, `session`, `account`, `verification`).
